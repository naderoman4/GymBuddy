      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8 bg-white overflow-y-auto">
        <div className="max-w-md w-full">
          <div className="mb-8">
            ...
            <p className="text-gray-600">{t('auth.startJourney')}</p>
          </div>
          ...
          {success && (
            <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
              ...
              <div>
                <p className="text-sm text-green-800 font-semibold">{t('auth.accountCreated')}</p>
                <p className="text-sm text-green-700 mt-1">{t('auth.redirectingToLogin')}</p>
              </div>
            </div>
          )}
                ...
                <div className="mt-3 space-y-2">
                  <div className={`flex items-center gap-2 transition-all duration-300 ${hasMinLength ? 'opacity-100' : 'opacity-60'}`}>
                    ...
                    <span className={`text-sm ${hasMinLength ? 'text-green-700 font-medium' : 'text-gray-600'}`}>
                      {t('auth.atLeast8Chars')} {hasMinLength && t('auth.niceWarmUp')}
                    </span>
                  </div>
                  ...
                  <div className={`flex items-center gap-2 transition-all duration-300 ${hasSpecialChar ? 'opacity-100' : 'opacity-60'}`}>
                    ...
                    <span className={`text-sm ${hasSpecialChar ? 'text-green-700 font-medium' : 'text-gray-600'}`}>
                      {t('auth.containsSpecialChar')} {hasSpecialChar && t('auth.strongLift')}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center">
              ...
              <Link to="/terms" className="text-blue-600 hover:underline">{t('auth.termsOfService')}</Link>
              {' '}{t('auth.and')}{' '}
              <Link to="/privacy" className="text-blue-600 hover:underline">{t('auth.privacyPolicy')}</Link>
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="absolute bottom-0 right-0 w-2/3 h-2/3 opacity-20">
          <svg viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {[...Array(28)].map((_, i) => {
              ...
              return (
                <g key={i}>
                  <circle cx={75 + col * 40} cy={155 + row * 40} r={hasEvent ? 8 : 3} fill="white" fillOpacity={hasEvent ? 0.4 : 0.15} />
                </g>
              )
            })}
          </svg>
